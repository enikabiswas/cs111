...
void free_block(void) {
	...
	int check = pread(fd, bitmap, blocksize, BLOCK_OFFSET(group_desc->bg_block_bitmap)); //get the actual bitmap
	errorcheck(check, "pread", "free_block");
	int i;
	for (i = 0; i < nblocks; i++) {
		int bcheck = bitmap[i];
		int j; 
...
void free_inode(void) {
	...
	uint8_t* bitmap = (uint8_t*) malloc(blocksize);
	int check = pread(fd, bitmap, blocksize, BLOCK_OFFSET(group_desc->bg_inode_bitmap));
	errorcheck(check, "pread", "free_inode");
	int i;
...
void indirect(int level, uint32_t owner, uint32_t offset, uint32_t blocknum) {
	...
	int check = pread(fd, block, blocksize, BLOCK_OFFSET(blocknum));
	errorcheck(check, "pread", "indirect");
...
void indirect_block_ref(struct ext2_inode* node, uint32_t owner_inode_num) {
	...
	if (node->i_block[13] != 0)
		indirect(2, owner_inode_num, 0, node->i_block[13]); //we pass 0 here as logical offset because we already account for it
...
//modeled off of:  http://cs.smith.edu/~nhowe/262/oldlabs/ext2.html
...
void print_directories(int level, int parent_num, void* block) {
	if (level == 0) {
		...
		while (size < blocksize) {
			...
			void* check = memcpy(file_name, dir_entry->name, dir_entry->name_len);
			if (check == NULL) {
				errorcheck(-1, "memcpy", "print_directories");
			}
		...
		if (sub_block == NULL)
			errorcheck(-1, "malloc", "print_directories");
		...
		for (j = 0; j < blocksize / 4; j++) {
			...
			if (*next_block_ptr != 0) {
				int check = pread(fd, sub_block, blocksize, blocksize * (*next_block_ptr));
				errorcheck(check, "pread", "print_directories");
			}
...
void directory(int parent_num, struct ext2_inode* node) {
	...
	if (block == NULL)
		errorcheck(-1, "malloc", "directory");
	int i;
	for (i = 0; i < 15; i++) {
		memset(block, 0, blocksize);
		int check = pread(fd, block, blocksize, BLOCK_OFFSET(node->i_block[i]));
		errorcheck(check, "pread", "directory");
		if (i < 12) {
...
void inode_summ(void) {
	...
	if (inodes == NULL)
		errorcheck(-1, "malloc", "inode_summ");
	int i;
	for (i = 0; i < n_inodes; i++) {     // we can just go through all of the inodes because we only have one group here
		...
		int check = pread(fd, node, INODE_SIZE, (i*INODE_SIZE) + BLOCK_OFFSET(group_desc->bg_inode_table)); 
		errorcheck(check, "pread", "inode_summ");
		...
		if (node->i_mode != 0 && (node->i_links_count) != 0) { //only want to print this if they are used/allocated
			...
			//for some reason, we need to call gmtime one at a time so that the times are correct
			//if we call all gmtimes first and then all strftimes then the times come out wrong
