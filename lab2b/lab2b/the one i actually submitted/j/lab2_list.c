...
void initialize(int nthreads) {
	...
	int j;
        for (j = 0; j < KEY_LEN; j++)
            key[j] = (char) rand() % 26 + 'A';  //  random ASCII chars
        key[KEY_LEN] = '\0';
        list_arr[i].key = key;
    }
    ...
    int val;
    for (i = 0, val = 0; i < nthreads; i++, val += niterations)
        start_poses[i] = val;
...
void* run_thread(void* pval) {
    ...
    int i;
    for (i = start_pos; i < upper_bound; i++) {
        ele = &list_arr[i];
...
int main(int argc, char *argv[]) {
    ...
    while ((ret = getopt_long(argc, argv, "", long_options, 0)) != -1) {
        switch (ret) {
            ...
            case OPT_YIELD:
                ...
                for (i = 0; i < len; i++) {  //  Set yield flags
                    if (optarg[i] == 'i') {
    ...
    //  Create N threads
    int start_pos;
    for (i = 0, start_pos = 0; i < nthreads; i++, start_pos += niterations) {
