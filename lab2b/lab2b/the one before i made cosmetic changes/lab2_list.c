...
void init_elements(int num_operations) {
	...
	for (x = 0; x < num_lists; x++) {
		SortedList_t *list = &((&multi_list[x])->list);
		list->key = NULL;
		list->next = list;
		list->prev = list;
		if (s_flag)
			(&multi_list[x])->spinlock = 0;
		if (m_flag)
			pthread_mutex_init(&(&multi_list[x])->lock, NULL);
	}	
	...
	//use different number seed for random function so we don't get the same
...
void *do_work(void* arg) {
	...
	for (i = startpos; i < endpos; i++) {
		...
		list_ptr = &(&multi_list[list_index])->list;
		if (m_flag) {
			lock = &(&multi_list[list_index])->lock;
			...
			spinlock = &(&multi_list[list_index])->spinlock;
			clock_gettime(CLOCK_MONOTONIC, &start);
