...
if __name__ == '__main__':
		...
		for line in reader:
			if line[0] == "SUPERBLOCK":
				...
				dirents.append(dirent(line))
                                if int(line[3]) != 2 and line[6] != "." and line[6] != "..": #these entries are special cases and mess up the mapping
                                        parents[int(line[3])] = int(line[1]) #key: child inode num, val: parent inode num
                                else:
                                        parents[2] = 2
	...
	for d in dirents:
                if d.name == "." and d.inode_of_refd != d.parent_inode:
                        print("DIRECTORY INODE {} NAME '{}' LINK TO INODE {} SHOULD BE {}".format(d.parent_inode, d.name, d.inode_of_refd, d.parent_inode))
                        code = 2
		if d.inode_of_refd != 2: 
                        if d.inode_of_refd in i_free:
                                print("DIRECTORY INODE {} NAME '{}' UNALLOCATED INODE {}".format(d.parent_inode, d.name, d.inode_of_refd))
                                code = 2
                        if d.name == "..":
                                if d.inode_of_refd != parents[d.parent_inode]:
                                        print("DIRECTORY INODE {} NAME '{}' LINK TO INODE {} SHOULD BE {}".format(d.parent_inode, d.name, d.inode_of_refd, d.parent_inode))
                                        code=2
                elif d.parent_inode == 2:
                        if d.name == ".." and d.inode_of_refd != d.parent_inode:
                                print("DIRECTORY INODE {} NAME '{}' LINK TO INODE {} SHOULD BE {}".format(d.parent_inode, d.name, d.inode_of_refd, d.parent_inode))
                                code = 2
	...
	for l in range(2, sup_block.total_inodes):
                if l in inodeDict and links[l] != inodeDict[l].link_count:
			print("INODE {} HAS {} LINKS BUT LINKCOUNT IS {}".format(l, links[l], inodeDict[l].link_count))
			code = 2
		if l in inodes and links[l] == 0:
			print("INODE {} HAS 0 LINKS BUT LINKCOUNT IS {}".format(l, inodeDict[l].link_count))
			code = 2
...
'''
        for d in dirents:
                ...
                if d.inode_of_refd < 0 or d.inode_of_refd > sup_block.total_inodes:
                        print("DIRECTORY INODE {} NAME '{}' INVALID INODE {}".format(d.parent_inode, d.name, d.inode_of_refd))
                        code = 2
                elif d.inode_of_refd not in i_free:
                        if d.name != "." and d.name != "..":
                                parents[d.inode_of_refd] = d.parent_inode
                else:
                        links[d.inode_of_refd] += 1
        ...
        for d in dirents:
                if d.name == "." and d.inode_of_refd != d.parent_inode:
                        print("DIRECTORY INODE {} NAME '{}' LINK TO INODE {} SHOULD BE {}".format(d.parent_inode, d.name, d.inode_of_refd, d.parent_inode))
                        code=2 
                if d.name == ".." and d.inode_of_refd != parents[d.parent_inode]:
                        print("DIRECTORY INODE {} NAME '{}' LINK TO INODE {} SHOULD BE {}".format(d.parent_inode, d.name, d.inode_of_refd, parents[d.parent_inode]))
                        code=2
