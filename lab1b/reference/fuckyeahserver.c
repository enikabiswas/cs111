	...
	if (childpid == 0) { //child process
		...
		while(1) {
			...
			if (pollfds[0].revents & POLLIN) {
				...
				printf("buf before decompression: %s\n", buf);
				//if receiving compressed input from shell
				...
				for (i = 0; i < strlen(buf); i++) {
					if (buf[i] == 0x03) {
						...
						printf("sent newline to shell");
					}
					else {
						int writebytes = write(tochild_pipefd[1], &buf[i], 1);
						errorcheck(writebytes, "write", "main");
						printf("sent %s to shell\n", &buf[i]);
					}
				}
			}
			memset(buf, 0, BUFSIZE);
			if (pollfds[1].revents & POLLIN) {
				...
				errorcheck(bytesread, "read", "main");
				printf("got from shell: %s\n", buf);
				int counter = 0;
				int i;
				for (i = 0; i < bytesread; i++) {
					if (buf[i] == 0x04) {
						//if sending compressed data to client
						if (compressflag == 1) {
							char* compressed_buf = NULL;
							compressed_buf = def(buf);
							int size = comp_size(compressed_buf);
							int check = write(newsockfd, compressed_buf, size);
						...
						{	int writebytes = write(newsockfd, buf, counter);
							errorcheck(writebytes, "write", "main");
						}
						exit(0);
					}
