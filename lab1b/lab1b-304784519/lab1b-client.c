...
#define BUFSIZE 1024
#define USAGE "usage: ./lab1b-server --port=port# [--log=logfile.txt] [--compress]\n"
#define SENT 1
#define RECEIVED 2
...
void updateLog(char *buf, int numbytes, int action) {
	...
	if (action == SENT) {
		...
		int check = write(logfd, message, size);
		errorcheck(check, "write", "updateLog");
	}
	...
	while(1) {
		...
		if (pollfds[0].revents & POLLIN) {
			...
			if (compressflag == 1) {
				...
				if (bytesread > 0) {
					updateLog(compressed_buf, size, SENT);
				}
			...
			for(i = 0; i < strlen(buf); i++) {
				if(buf[i] == '\r' || buf[i] == '\n') {
					...
					errorcheck(writecheck, "write", "main");
				}
				else {
