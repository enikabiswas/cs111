	...
	while(1) {
		...
		if (pollfds[0].revents & POLLIN) {
			...
			for(i = 0; i < bytesread; i++) {
				//TODO: compress byte at a time right here and put into a 10 byte or so buffer 
				//so that you can store stuff and then write that to socket isntead
			...
			//TODO: do compression before updating log when sending
			//--> make the compression within the for loop
			if (bytesread > 0) {
		...
		if (pollfds[1].revents & POLLIN) {
			...
			//just deflate buf here and everything else should be fine
			int i;
			for (i = 0; i < bytesread; i++) {
				if (buf[i] == '\n') {
					write(STDOUT_FILENO, cr_lf, 2);
